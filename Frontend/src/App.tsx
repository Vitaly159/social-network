...
function App() {
  ...
  const [currPath, setCurrPath] = useState(localStorage.currPath ? JSON.parse(localStorage.currPath) : []);

  useEffect(()=>{
    currPath !== '/' && localStorage.setItem("currPath", JSON.stringify(currPath))
  }, [currPath])
  ...
  return (
    <Box>
      <Routes>
        ...
        <Route path={routing("/home", isAuth)} element={<Home />} />
      </Routes>
    </Box>
  );
}
